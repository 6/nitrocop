...
///   sends.
/// - FN=2 under `EnforcedStyle: return_nil`: inner `return` nodes used inside another
///   return's argument expression, like `return value && return`, were skipped because
///   `visit_return_node` never recursed into return arguments. Fixed by visiting nested
///   return arguments after checking the current node so both returns are analyzed.
...
mod tests {
    ...
    fn return_nil_style_offense_fixture() {
        let fixture =
            include_bytes!("../../../tests/fixtures/cops/style/return_nil/offense.return_nil.rb");
