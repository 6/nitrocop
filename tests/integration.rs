...
#[test]
fn only_rails_unique_validation_without_index_does_not_enable_unloaded_plugin_department() {
    ...
    assert!(
        ...
        "--only should not force-enable Rails cops when the repo config does not load rubocop-rails: {stdout}"
    );
